...
def remove_extra_slashes(selector):
    if '\\"' in selector:
        if selector.count('\\"') == selector.count('"'):
            ...
            selector.count('\\"') == selector[1:-1].count('"')
            and "'" not in selector[1:-1]
        ):
...
def create_objects_file(selector_list_dict=None):
    data = (
        ...
        else ["", "class Page(object):", '    html = "html"', ""]
    )

    if selector_list_dict:
        for key, pairs in selector_list_dict.items():
            if key == "None":
                if pairs:
                    data.extend(f"{pair[0]} = {pair[1]}" for pair in pairs)
                    data.append("")
            else:
                data.append(f"\nclass {key}(object):")
                data.extend(f"    {pair[0]} = {pair[1]}" for pair in pairs)
    ...
    with open(PAGE_OBJECTS_FILE, "w", encoding="utf-8") as file:
        file.write("\n".join(data))

    if not selector_list_dict:
        print(f'\n>>> ["{PAGE_OBJECTS_FILE}"] was created!')
    else:
        print(f'\n>>> ["{PAGE_OBJECTS_FILE}"] was updated!')
...
def scan_objects_file():
    ...
    selectors = []
    current_class = "None"
    selector_list_dict = {"None": [], "Page": []}

    class_pattern = re.compile(r"""^(\s*)class\s+([\S]+)\((object|)\):\s*$""")
    selector_pattern = re.compile(r"""^(\s*)(\S+)\s*=\s*([\S\s]+)\s*$""")
    ...
    for line in code_lines:
        ...
        if class_match:
            ...
            page_selectors[f"class {current_class}"] = "."
            continue

        selector_match = selector_pattern.match(line)
        if selector_match:
            name, selector = selector_match.group(2), remove_extra_slashes(
                selector_match.group(3)
            )
            page_selectors[name] = selector
            var_names.append(name)
            selectors.append(selector)
            if not selector_match.group(1):
                current_class = "None"
    ...
    if not selector_list_dict["Page"]:
...
def process_test_file(
    ...
    for line in code_lines:
        ...
        if not object_dict:
            data = re.match(
                r"""^(\s*)self\.(\S*)_click""" r"""\((r?['"][\S\s]+['"])\)([\S\s]*)$""",
                line,
            )
        else:
            data = re.match(
                r"""^(\s*)self\.(\S*)_click""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            data = re.match(
                r"""^(\s*)self\.click""" r"""\((r?['"][\S\s]+['"])\)([\S\s]*)$""",
                line,
            )
        else:
            data = re.match(
                r"""^(\s*)self\.click""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.click_if_visible""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            data = re.match(
                r"""^(\s*)self\.hover""" r"""\((r?['"][\S\s]+['"])\)([\S\s]*)$""",
                line,
            )
        else:
            data = re.match(
                r"""^(\s*)self\.hover""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.highlight""" r"""\(([\S]+),([\S\s]*)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            data = re.match(
                r"""^(\s*)self\.highlight""" r"""\((r?['"][\S\s]+['"])\)([\S\s]*)$""",
                line,
            )
        else:
            data = re.match(
                r"""^(\s*)self\.highlight""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.check_if_unchecked""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.uncheck_if_checked""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.select_if_unselected""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.unselect_if_selected""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.switch_to_frame""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)(\S*)\sself\.frame_switch""" r"""\(([\S]+)\):([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.assert_element_(\S*)""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.wait_for_element_(\S*)""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.add_text""" r"""\(([\S]+),\s?([\S\s]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.send_keys""" r"""\(([\S]+),\s?([\S\s]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.set_value""" r"""\(([\S]+),\s?([\S\s]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.press_(\S*)_arrow""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.find_text""" r"""\(([\S\s]+),\s?([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)(\S*)\sself\.is_selected""" r"""\(([\S]+)\):([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)(\S*)\s?=\s?self\.get_text""" r"""\(([\S]+)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
        ...
        if not object_dict:
            ...
            data = re.match(
                r"""^(\s*)self\.(\S*)_element""" r"""\(([\S]+),([\S\s]*)\)([\S\s]*)$""",
                line,
            )
        if data:
            whitespace = data.group(1)
